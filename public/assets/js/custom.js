...
document.addEventListener('DOMNodeInserted', function(x) {
  $(document).ready(function(e) {
      ...
      if (product_detail_el.length == 1) {
        ...
        trigger_el.on('click', function(e) {
          ...
          if (
            ...
            product_details_container.offset().top
          ) {
    ...
    if (main_navigation_el.length == 1) {
      ...
      window_el
        ...
        .on('resize', function(e) {
          ...
          } else main_nav_placeholder_el.height(main_navigation_el.outerHeight());
        });
    }
    ...
    for (var i = 0, len = menu_items.length; i < len; i++) {
      ...
      if (submenu.length == 1) submenuHandler(menu_items.eq(i), submenu);
    }
    ...
    for (var i = 0, len = product_pieces.length; i < len; i++) {
      if (product_pieces.eq(i).hasClass('product-pieces-readonly')) continue;
  ...
  $(window).load(function() {
    ...
    } else page_loader_el.addClass('page-loader-hidden');
    ...
    for (var i = 0; i < parallax_backgrounds.length; i++) {
      ...
      if (!el.attr('data-stellar-background-ratio')) el.attr('data-stellar-background-ratio', 0.4);
    }
...
*/
  ...
  function isSet(variable) {
    if (variable == '' || typeof variable == 'undefined') return false;
    return true;
  }
  ...
  function setReturnMessage(form_el, content) {
    if (!isSet(content)) content = 'Unspecified message.';
